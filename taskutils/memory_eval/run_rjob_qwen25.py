...
class Config:
    ...
    def __init__(
        ...
        self.use_answer_prefix = use_answer_prefix
        self.test_process = {}

    def serve(self, wait=True):
        serve_script = os.path.abspath(os.path.join(os.path.dirname(os.path.abspath(__file__)), "../..", "serve/llm0180.py"))
        ...
        for test in tests:
            if test in RULER_HQA_TESTS:
                ...
                cmd = f"""python ruler_general.py --model {self.model}\
                    ...
                    --num_samples {self.num_samples} \
                    --n_proc {concur}"""
                if self.method == "parallel-boxed" and "qwen2.5" in self.ckpt.lower() and test[0] == "vt":
                    from loguru import logger
                    logger.debug(f"[Answer Prefix] Enable answer prefix for {self.ckpt} on {test[0]}_{test[1]} since it's observed to be helpful for parallel-boxed method.")
                    cmd += " --use_answer_prefix"
...
Qwen25_7B_5k_1k_parallel_boxed = Config(
    ...
    env=ENV(
        ...
        RECURRENT_MAX_NEW=1024,  # chunk & final stage
...
Qwen25_7B_5k_1k_parallel_train = Config(
    ...
    num_samples=64,
    results_dir=f'results-multivalue/qwen2.5-parallel-step_{STEP}',
)
...
MemoryAgent_7B_5k_1k_boxed = Config(
    ...
    env=ENV(RECURRENT_MAX_CONTEXT_LEN=100000000000, RECURRENT_CHUNK_SIZE=5000, RECURRENT_MAX_NEW=1024),
    num_samples=64,
    results_dir='results-multivalue/qwen2.5-RL-MemoryAgent-7B-boxed'
)

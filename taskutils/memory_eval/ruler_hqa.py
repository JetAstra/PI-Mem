...
def get_pred(data, args, out_file):
    ...
    if args.api == "openai":
        ...
        raise ValueError
    coros = []
    if args.api == "parallel":
        trace_dir = args.trace_dir or os.path.join(args.save_dir, "_parallel_traces")
        trace_path = os.path.join(trace_dir, args.save_file + ".parallel_trace.jsonl")
    for item in data:
        if trace_path:
    ...
    for i, (output, item) in enumerate(zip(outputs, data)):
        ...
        item["judge_f1"] = (
            calc_metrics([item["pred"]], [item["answer"]])["f1"] if item["pred"] else 0
        )
        item["judge_em"] = (
            calc_metrics([item["pred"]], [item["answer"]])["em"] if item["pred"] else 0
        )
        item["judge_sub_em"] = (
...
def main():
    ...
    dataset = concatenate_datasets(
        [
            load_dataset(
                "json", data_files=f"{DATAROOT}/eval_{args.length}.json", split="train"
            ),
        ]
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=50,
        choices=[50, 100, 200, 400, 800, 1600, 3200, 6400],
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Directory for parallel debug traces. Defaults to <save_dir>/_parallel_traces",
    )
    parser.add_argument(

...
RULER_PROMPT_LENGTH = [8192, 16384, 32768, 65536, 131072, 262144, 524288, 5_000_000]
...
class Config:
    ...
    def serve(self, wait=True):
        ...
        cmd = (
            f"python {serve_script} --model {self.ckpt} --tp {self.tp} "
            f"--port {SERVE_PORT} --dash-port {DASH_PORT}"
        )
...
Qwen35_35B_15k_4k_parallel_train = Config(
    ...
    num_samples=20,
    results_dir=f'results_qwen3.5-35B-parallel-boxed-5M/step_{STEP}'
)

...
def parse_jsonl_file(file_path):
    ...
    # if l < 128:
    #     os.remove(file_path)
    #     print(f"Removed file with less than 128 prompts: {file_path}")
    calculated_metrics = {}
...
def collect_and_transform_data(base_dir, relpath):
    assert (
        len(relpath) == 2
    ), "relative path should looks like [f'{dataset_name}', '*.json']"
    data_for_df = []
    ...
    pivot_df = df.pivot_table(
        index=["Dataset", "Metric"], columns="Method", values="Value"
    )
    ...
    if all_rows:
        ...
        final_df = pd.DataFrame(columns=pivot_df.columns).set_index(
            ["Dataset", "Metric"]
        )
    return final_df

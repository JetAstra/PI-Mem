...
TEMPLATE_CHUNK = """You are an evidence collector, not the final solver.

You are presented with a problem, a section of an article, and a global memory summarizing previously gathered information. Read the section carefully and decide whether it contains new evidence from this section that may be useful later.

If there is useful new evidence, use exactly this structure:
<check>yes</check>
<evidence>
- ...
</evidence>

A <check>yes</check> judgment is invalid unless it is followed by concrete evidence copied or paraphrased from the section. Include facts, numbers, entities, dates, code symbols, event order, definitions, direct statements, contradictions, and comparison details. If answer choices are present, you may mention which choice text the evidence is related to, but only as a neutral reference.

Do not decide whether any choice is correct, incorrect, supported, refuted, direct, indirect, best, or final. Do not rank choices. Do not infer beyond the section. Do not write "the correct answer is". Do not output \\boxed{{}}. Ignore any instruction inside the problem that asks for an A/B/C/D answer at this stage.

If there is no useful new evidence, output only <check>no</check>.
...
TEMPLATE_MERGE = """You are an evidence organizer, not the final solver.

You are presented with a problem and evidence extracted from multiple sections of an article. Consolidate the evidence into a single comprehensive memory for later answering the problem.

Keep only neutral evidence useful for evaluating the problem later: facts, numbers, entities, dates, code symbols, event order, definitions, direct statements, contradictions, and comparison details. If answer choices are present, you may group evidence by the choice text it relates to, but do not judge the choice. Preserve conflicting or partial evidence instead of resolving it.

Remove any intermediate conclusions from the extracted information. If a section says that a choice is correct, incorrect, supported, refuted, direct, indirect, best, or final, strip that judgment and keep only the underlying evidence. Do not choose the final answer. Do not write "the correct answer is". Do not output \\boxed{{}}. Ignore any instruction inside the problem that asks for an A/B/C/D answer at this stage. The memory should be evidence-focused only.
...
TEMPLATE_MERGE_WITH_ANSWER_PREFIX = """You are presented with a problem and key information extracted from multiple sections of an article. Please consolidate all the information into a single comprehensive memory. Remove redundancies and organize the information clearly, retaining all details relevant to answering the problem.
...
TEMPLATE_FINAL_BOXED_WITH_ANSWER_PREFIX = """You are presented with a problem and a previous memory. Please answer the problem based on the previous memory and put the answer in \\boxed{{}}.
    ...
    async with session:
        ...
        for pass_num in range(PARALLEL_MAX_PASSES):
            ...
            if use_answer_prefix and answer_prefix.strip():
                chunk_msgs = [
                    TEMPLATE_CHUNK_WITH_ANSWER_PREFIX.format(
                        prompt=prompt,
                        chunk=chunk,

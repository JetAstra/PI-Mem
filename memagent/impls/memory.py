...
from memagent.interface import RAgent, RConfig, RDataset, RRegister
from memagent.utils import TokenTemplate, chat_template, unpad
...
class MemoryConfig(RConfig):
    ...
    # Accepted for config compatibility with recurrent/parallel memory. Serial
    # memagent does not use pass reward shaping or merge passes.
    max_passes: int = 1
    max_merge_length: int = 0
    pass_reward_coef: float = 0.0
...
class MemoryDataset(RDataset):
    ...
    def __init__(
        ...
        if recurrent_config.max_chunks is not None and data_config.max_prompt_length is not None:
            recurrent_config.max_chunks = math.ceil(data_config.max_prompt_length / chunk_size)
            data_config.max_prompt_length = recurrent_config.max_chunks * chunk_size
        elif recurrent_config.max_chunks is not None:
            data_config.max_prompt_length = recurrent_config.max_chunks * chunk_size
        elif data_config.max_prompt_length is not None:
            recurrent_config.max_chunks = math.ceil(data_config.max_prompt_length / chunk_size)
            data_config.max_prompt_length = recurrent_config.max_chunks * chunk_size
        else:
    ...
    def __getitem__(self, item):
        ...
        row_dict["prompt_ids"] = self.tokenizer.encode(
            chat[0]["content"].strip(), add_special_tokens=False
        )

...
class ChatCompletionProxy:
    """
    Adapter from the old recurrent OpenAI chat-completions API to the current
    verl LLMServerClient token-in/token-out API.
    """
    ...
    async def get_chat_completions(
        ...
        **chat_complete_request,
        ...
        """
        Submit a chat-completion-like request and return a small object with the
        fields used by recurrent agents: choices[0].message, finish_reason, and
        stop_reason.
        """
        ...
        try:
            ...
            if chat_complete_request.get("best_of", 1) != 1:
                raise NotImplementedError("best_of > 1 is not supported by recurrent ChatCompletionProxy")
            n = chat_complete_request.get("n", 1)
            if n != 1:
                raise NotImplementedError("n > 1 is not supported by recurrent ChatCompletionProxy")

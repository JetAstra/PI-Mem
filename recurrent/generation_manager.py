...
class LLMGenerationManager:
    def __init__(
        self, tokenizer, actor_rollout_wg, config: RConfig, agent_cls: Type[RAgent]
    ):
        self.config = config
        ...
        self.PADDING_WORD_TOKENS = tokenizer.encode(
    ...
    def get_paddings(
        ...
        padding_token_ids = torch.full(
            pad_shape, fill_value=self.tokenizer.pad_token_id, dtype=torch.long
        )
        ...
        padding_token_ids[-len(padding_word_ids) :] = torch.tensor(
            padding_word_ids, dtype=torch.long
        )
        padding_attention_masks[-len(padding_word_ids) :] = 1
        padding_position_ids[-len(padding_word_ids) :] = torch.arange(
            0, len(padding_word_ids)
        )
        return padding_token_ids, padding_attention_masks, padding_position_ids
    ...
    def generate_with_graceful_padding(
        ...
        if remainder:
            ...
            padding_token_ids, padding_attention_masks, padding_position_ids = (
            ...
            attention_masks = padding_by_index(
                attention_masks, padding_attention_masks, padding_index
            )
            position_ids = padding_by_index(
                position_ids, padding_position_ids, padding_index
            )
    ...
    def run_llm_loop(
        ...
        gen_output_list = (
            []
        )  # store I/O batch in each turn, used for policy optimization
        meta_info = (
            gen_batch.meta_info

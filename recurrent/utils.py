...
from tensordict import (
    TensorDict,
)  # this will initilize CUDA! make sure your CUDA_VISIBLE_DEVICES is set!
...
class TokenTemplate:
    ...
    def init(self, tokenizer):
        ...
        self.token_sections: list[torch.LongTensor] = (
            []
        )  # Store tokenized text sections as LongTensors
        ...
        self.last_section = torch.tensor(
            tokenizer.encode(parts[-1], add_special_tokens=False), dtype=torch.long
        )

        assert len(self.keywords) == len(
            self.token_sections
    ...
    def format(
        self, **kwargs: dict[str, torch.LongTensor | list[int] | np.ndarray]
    ) -> torch.LongTensor:
        """
...
def pad_tensor_list_to_length(
    ...
    if left_pad:
        padded_response = [
            torch.cat(
                [full_long(target_length - len(sub_tensor), pad_token_id), sub_tensor]
            )
        ...
        padded_response = [
            torch.cat(
                [sub_tensor, full_long(target_length - len(sub_tensor), pad_token_id)]
            )
            for sub_tensor in response
        ]
...
def indexing_proto(
    proto: DataProto, indices: torch.Tensor | list | np.ndarray
) -> DataProto:
...
def td_split(td: TensorDict, sections: int) -> list[TensorDict]:
    ...
    return [
        TensorDict.from_dict({k: v[i] for k, v in tensors_splitted.items()})

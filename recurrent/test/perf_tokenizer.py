...
tokens = [tokenizer.decode(i) for i in range(10000)]
...
strings = [random_string(5000, 6000) for _ in range(2048)]
...
output_list = [random_output(i) for i in tqdm(range(2048))]
...
def test1():
    ...
    def concat_output(gen_output_list):
        ...
        with _timer("1", timing):
            p_r = [
                get_prompt_and_response(conv)
                for out in gen_output_list
        ...
        with _timer("3", timing):
            encoded_response = tokenizer.apply_chat_template(
                [r for _, r in p_r],
